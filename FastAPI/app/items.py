...
from schema import Item, StoreName, inventory
...
async def get_item_by_store(
    store_name: StoreName, product_type: str = Query(None, description="Type of item")
):
    keys = [
        k
        for k, v in inventory.items.items()
        if v["store_name"] == store_name and v["product_type"] == product_type
    ]
    if keys:
        return [inventory.items.get(key) for key in keys]
